define(["app", "moment"], function(app, moment) {
	app.controller("DashboardCtrl", ["$rootScope", "$scope", "$filter", "$interval", "$timeout", "$compile", "ngDialog", "dashboardService", "uiGridConstants", "ConfigManager", "AlarmManager",
		function($rootScope, $scope, $filter, $interval, $timeout, $compile, ngDialog, dashboardService, uiGridConstants, ConfigManager, AlarmManager) {
			"use strict";
			...
			var historyLabel = "한 주 변화 추이 정보 ";
			var serviceDataLabe1 = " and ";
			var serviceDataLabe2 = " additional Service";
			...
			dashboardCtrl.clickService = function ($event, service) {
				...
				if(angular.equals(dashboardCtrl.selectedServiceList, [service])) {
					...
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].serviceName);
					}
					...
					// var service_list = dashboardCtrl.selectedServiceList.concat(service);
					...
					// dashboardCtrl.alarmFilter = [service];
					setAlarmFilter(service_list);

					// setSelectedService([service]);
					// setSelectedVolume(getVolumeByService(service));
					// setSelectedHost(getHostByVolumeList(getVolumeByService(service)));
					setSelectedService(service_list);
					setSelectedVolume(getVolumeByServiceList(service_list));
					setSelectedHost(getHostByVolumeList(getVolumeByServiceList(service_list)));
				}
			...
			dashboardCtrl.clickVolume = function ($event, volume) {
				...
				if(angular.equals(dashboardCtrl.selectedVolumeList, [volume])) {
					...
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].volumeName);
					}
					...
					console.log("dashboard : clickVolume : [[[[[ volume 2 ]]]]]", dashboardCtrl.clickedResourceList);
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].volumeName);
					}
					...
					console.log("dashboard : clickVolume : [[[[[ volume 3 ]]]]]", dashboardCtrl.clickedResourceList);
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].volumeName);
					}
					...
					// dashboardCtrl.alarmFilter = [volume];
					setAlarmFilter(volume_list);

					// setSelectedService(getServiceByVolume(volume));
					// setSelectedVolume([volume]);
					// setSelectedHost(getHostByVolume(volume));
					setSelectedService(getServiceByVolumeList(volume_list));
					setSelectedVolume(volume_list);
					setSelectedHost(getHostByVolumeList(volume_list));
				}
			...
			dashboardCtrl.clickHost = function ($event, host) {
				...
				if(dashboardCtrl.selectedHostList && angular.equals(dashboardCtrl.selectedHostList, [host])) {
					...
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].hostName);
					}
					...
					console.log("dashboard : clickHost : [[[[[ host 2 ]]]]]", dashboardCtrl.clickedResourceList);
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].hostName);
					}
					...
					console.log("dashboard : clickHost : [[[[[ host 3 ]]]]]", dashboardCtrl.clickedResourceList);
					for(var idx in dashboardCtrl.clickedResourceList) {
						console.log(idx, dashboardCtrl.clickedResourceList[idx].hostName);
					}
					...
					// dashboardCtrl.alarmFilter = [host];
					setAlarmFilter(host_list);

					// setSelectedService(getServiceByHost(host));
					// setSelectedVolume(getVolumeByHost(host));
			...
			dashboardCtrl.clickHostList = function (id) {
				...
				var selector = $("#list_"+id);
				$(".host-list .active").removeClass("active");
				...
				$timeout(function(){
					...
					$(".host-content-table").removeClass("active");
			...
			function initializeEventHandler() {
				unbind = [$rootScope.$on(ConfigManager.getEvent("OCCURED_ALARM"), onOccuredAlarm),
			...
			/*function getAlarmList() {
				return AlarmManager.getAlarmList(1, 100).then(function(e) {
					console.log("dashboard : AlarmManager.getAlarmList :", e);
					if (e && e.datas) {
						// TODO. alarm list
					}
				});
			...
			function getAlarmStatus() {
				return dashboardService.getAlarmStatus().then(function (e) {
					...
					if(e) {
						dashboardCtrl.statusService = { normalCount: parseInt(e.svcAlarmTotal) - parseInt(e.svcAlarmCount), abnormalCount: parseInt(e.svcAlarmCount) };
						dashboardCtrl.statusVolume = { normalCount: parseInt(e.volAlarmTotal) - parseInt(e.volAlarmCount), abnormalCount: parseInt(e.volAlarmCount) };
						dashboardCtrl.statusHost = { normalCount: parseInt(e.hostAlarmTotal) - parseInt(e.hostAlarmCount), abnormalCount: parseInt(e.hostAlarmCount) };
			...
			function getHighestAlarmStatus() {
				return dashboardService.getHighestAlarmStatus().then(function (e) {
					...
					if(e && e.length) {
						dashboardCtrl.highestAlarmStatus = e;
					} else {
						dashboardCtrl.highestAlarmStatus = [];
					}
				})
			...
			function drawSelectHostDetail() {
				...
				for(var i=0, il=dashboardCtrl.selectedHostList.length; i<il; ++i) {
					for(var j=0, jl=dashboardCtrl.statusHostList.length; j<jl; ++j) {
						if (dashboardCtrl.selectedHostList[i].hostName == dashboardCtrl.statusHostList[j].hostName ) {
							$(".host-list").append($compile('<ul><li><a class="host-list-item" id="list_' + i + '" href="javascript:;" ng-click="dashboardCtrl.clickHostList(' + i + ')">' + dashboardCtrl.statusHostList[j].hostName + '</a></li></ul>')($scope));
							$(".host-content").append('<div id="content_' + i + '" class="host-content-table">');
							$("#content_" + i).append(dashboardCtrl.statusHostList[j].summary);
							break;
						}
					}
				}
				...
				if ($(".host-content-table").length > 0) {
					$($(".host-content-table")[0]).addClass("active");
				}
			...
			function setSelectedService(service_list) {
				...
				// if(angular.equals(dashboardCtrl.selectedServiceList, service_list))	return;
			...
			function setSelectedVolume(volume_list) {
				...
				// if(angular.equals(dashboardCtrl.selectedVolumeList, volume_list))	return;
				...
				for(i=0; i<dashboardCtrl.statusVolumeList.length; ++i) {
					// for(var j=0; j<dashboardCtrl.selectedVolumeList.length; ++j) {
			...
			function isSelectedService(service) {
				for(var i=0; i<dashboardCtrl.selectedServiceList.length; ++i) {
					if(service.serviceName == dashboardCtrl.selectedServiceList[i].serviceName) {
						return true;
					}
				}
				return false;
			...
			function getVolumeByService(service) {
				var i, il;
				var select_service;
				for(i=0, il=dashboardCtrl.statusServiceList.length; i<il; ++i) {
					if(service.serviceName.toUpperCase() == dashboardCtrl.statusServiceList[i].serviceName.toUpperCase()) {
						select_service = dashboardCtrl.statusServiceList[i];
						break;
					}
				}

				var list = [];
				for(i=0, il=dashboardCtrl.statusVolumeList.length; i<il; ++i) {
					if(select_service.volumeName.toUpperCase() == dashboardCtrl.statusVolumeList[i].volumeName.toUpperCase()) {
						list.push(dashboardCtrl.statusVolumeList[i]);
					}
				}
				return list;
			}
			...
			function getVolumeByHost(host) {
				...
				// console.log(selected_volumn_list);
				/*for (var idx in selected_volumn_list) {
					console.log(resourceType[1], idx, (selected_volumn_list[idx].volumeName ? selected_volumn_list[idx].volumeName : 'null'));
				}*/
			...
			function getHostByVolume(volume) {
				...
				// console.log(selected_host_list);
				/*for (var idx in selected_host_list) {
					console.log(resourceType[2], idx, (selected_host_list[idx].hostName ? selected_host_list[idx].hostName : 'null'));
				}*/
			...
			function getServiceByName(name) {
				for(var i=0, il=dashboardCtrl.statusServiceList.length; i<il; ++i) {
					if(name.toUpperCase() == dashboardCtrl.statusServiceList[i].serviceName.toUpperCase()) {
						return dashboardCtrl.statusServiceList[i];
			...
			function getGridOption(data) {
				return {
					...
					columnDefs: [
						...
						// { name: 'systemGroup', enableColumnMenu: false, cellTooltip: true, headerTooltip: true },
						{ name: 'serviceName', enableColumnMenu: false, cellTooltip: true, headerTooltip: true },
						// { name: 'serviceId', enableColumnMenu: false, cellTooltip: true, headerTooltip: true },
						...
						// { name: 'upTime', enableColumnMenu: false, cellTooltip: true, headerTooltip: true }
						// { name: 'delFlag', enableColumnMenu: false, cellTooltip: true, headerTooltip: true }
					],
			...
			function selectScrolling(className, scrollRect, margin, complete) {
				...
				var y= $(e[0]).position().top - $(s[0]).position().top + $(s[0]).scrollTop() - margin;
				if (y < 0) {
					y = 0;
				}
			...
			function removeNodeDetailScrollSyncToNodeList() {
				$(".host-content.mu-scroll-v").unbind('scroll');
			}
			function setNodeDetailScrollSyncToNodeList() {
				$(".host-content.mu-scroll-v").scroll(function(e) {
					var scroll = $(e.target);
	            	var st = scroll.scrollTop();
	            	var bh = scroll.position().top;
	            	var page = 0;
	            	
	            	var l = $(".host-content-table");
	            	if (st > 10) {
					...
					var hl = $(".host-list-item");
					if (hl.length == 0) {
						return;
					}
					
					hl.removeClass("active");
					$(hl[page]).addClass("active");
				});
			}
			...
			if($(".mu-dialog").css("display") != "none") {
				if(value) {
					...
					if(self.dataType == "service") {
						...
						dashboardService.getSelectServiceDetail({serviceId:value.serviceId, serviceType:value.serviceType}).then(function (data) {
						...
						// self.style = {'height': '430px', 'width':'600px' };
