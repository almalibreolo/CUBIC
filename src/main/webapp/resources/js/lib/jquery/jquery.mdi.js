    ...
    $.fn.extend({
        mdi: function(options) {
            ...
            function makeWindowByHtml(id, title, html, width, height, modal, sub) {
                ...
                var body = ["<div id='", id, CLASS_SUB_BODY, "' class='sub-body' width='100%' height='100%' scrollbar-policy='auto' gap='2px'></div>"];
            ...
            function resizableSubWindow(subWindow) {
                subWindow.window.resizable({
                    ...
                    stop: function(e, ui) {
                        $(this).attr({"height": ui.helper.outerHeight() + "px", "width": ui.helper.outerWidth() + "px"});
            ...
            function closeAll() {
                ...
                for (var i = 0; i < l; i++) {
                	...
                	windowList[i].remove();
                    $(windowList[i]).trigger(EVENT_CLOSE, windowList[i]);
                }     
