...
public class SessionManager {
	 ...
	 * @throws Exception
	 */
	public IUserModel auth(HttpServletRequest request, HttpServletResponse response, IUserModel userModel, IUserDao userDao) {
		IUserModel user = null;
		try {
			// 사용자 인증 시작
			user = userDao.getUser(userModel);
			if (user == null) {
			     throw new AuthenticationCredentialsNotFoundException("User or Password not found.");
			}
			
			// BCrypt 체크
			if (!CryptUtil.checkpw(userModel.getPassword(), user.getPassword())) {
				throw new AuthenticationCredentialsNotFoundException("User or Password not found.");
			}
